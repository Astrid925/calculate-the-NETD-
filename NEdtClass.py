...
class NEdt():
    def __init__(self,filePath1,filePath2,imgW,imgH,T1,T2,frameNum,radius,auxData_flag):
    ...
    def NEdt_cal(self,NDT1,NDT2,T1,T2,radius,imgW,imgH):
        ...
        if radius==0:
            ...
            imgMask=self.generate_mask(imgH, imgW, radius, imgW/2, imgH/2)
            rNEdt=imgMask*NEdt
            averNEdt = np.mean(rNEdt)
            stdNEdt = np.std(rNEdt)
            maxNEdt = np.max(rNEdt)
            minNEdt = np.min(rNEdt)
    ...
    def dataProcessing(self):
        ...
        if self.auxData_flag == 1:
            ...
            NDT1Data = np.zeros((self.imgH - 1, self.imgW - 8, nFrame//self.frameNum), dtype=np.uint16, order="C")
            NDT2Data = np.zeros((self.imgH - 1, self.imgW - 8, nFrame//self.frameNum), dtype=np.uint16, order="C")
            for i in range(0, nFrame):
                ...
                NDT1[:, :, i] = temp1[1:, 7:-1]
                NDT2[:, :, i] = temp2[1:, 7:-1]
            for i in range(0,nFrame//self.frameNum):
                NDT1Data[:,:,i]=np.mean(NDT1[:,:,i*self.frameNum:(i+1)*self.frameNum],axis=2)
                NDT2Data[:, :, i] = np.mean(NDT2[:, :, i * self.frameNum:(i + 1) * self.frameNum], axis=2)

            (self.NEdt, self.averNEdt, self.stdNEdt, self.maxNEdt, self.minNEdt) = self.NEdt_cal(NDT1Data, NDT2Data, self.T1,self.T2, self.radius,self.imgW-8, self.imgH-1)
        else:
            nFrame1 = int(fileSize1 / (self.imgW * self.imgH * 2))
            nFrame2 = int(fileSize2 / (self.imgW * self.imgH * 2))
            if nFrame1 >= nFrame2:
            ...
            for i in range(0,nFrame//self.frameNum):
                NDT1Data[:,:,i]=np.mean(NDT1[:,:,i*self.frameNum:(i+1)*self.frameNum],axis=2)
                NDT2Data[:, :, i] = np.mean(NDT2[:, :, i * self.frameNum:(i + 1) * self.frameNum], axis=2)
            (self.NEdt, self.averNEdt, self.stdNEdt, self.maxNEdt, self.minNEdt) = self.NEdt_cal(NDT1Data, NDT2Data, self.T1, self.T2,self.radius,self.imgW,self.imgH)
...
if __name__=="__main__":
    ...
    auxData_flag = 1
    myTest=NEdt(filePath1,filePath2,imgW,imgH,T1,T2,nFrame,radius,auxData_flag)
    myTest.openFile()
